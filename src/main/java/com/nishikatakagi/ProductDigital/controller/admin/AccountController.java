...
public class AccountController {
    ...
    public String showPagePublisher(Model model) {
        List<User> listAccount = userService.findAllUser();
    ...
    public String deleteAccount(@RequestParam("username") String username, RedirectAttributes model) {
        ...
        if(userDelete.getRoleId()==1){
            ...
            return "redirect:/account";
        }else{userService.deleteUserByAdmin(username,user);
            return "redirect:/account";
        }
    }
    ...
    @PostMapping("/create")
    public String createAccount(Model model,@Valid @ModelAttribute("user") UserRegisterRequestDto user, BindingResult result) {
    ...
    public String showDetailAccount(Model model,@RequestParam("username") String username,RedirectAttributes redirectAttributes) {
    ...
    public String showChangePasswordPage(Model model,@RequestParam("username") String username,RedirectAttributes redirectAttributes) {
        User user = userService.findByUsername(username);
        if(user.getRoleId()==1){
    ...
    @GetMapping("/filter")
    public String filterAccount(@RequestParam(value="role",required=false) List<Integer> roleId,@RequestParam(value="is-deleted",required=false) List<Integer> isDeleted,@RequestParam(value = "username",required = false) String username,Model model) {
        List<User> listAccount = userService.filterAccount(roleId,isDeleted,username);
        model.addAttribute("listAccount", listAccount);
